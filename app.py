...
def load_models():
    ...
    try:
        face_model = tf.keras.models.load_model("dataset/facial emotion/face_emotion_model_savedmodel")
    except Exception as e:
        st.error(f"CRITICAL: Failed to load face model. Please check the path. Error: {e}")
    
    try:
        audio_model = tf.keras.models.load_model("dataset/voice emotion/Speech-Emotion-Recognition-Model")
    except Exception as e:
...
EMOTION_LABELS = ['Angry', 'Disgust', 'Fear', 'Happy', 'Sad', 'Neutral','Suprise']
